...
func (gb *GitlabManager) GetGroupProjects(groups []string) ([]*template.GroupTemplate, error) {
	configTemplate := []*template.GroupTemplate{}
	...
	for _, group := range gb.groups {
		...
		go func(group *gitlab.Group) {
			...
			var projects []template.ProjectTemplate
			for _, project := range p {
				projects = append(projects, template.ProjectTemplate{Url: project})
			}
